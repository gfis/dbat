...
<!--
    ...
    @(#) $Id$
    2016-05-24: PROCEDURE ... IF EXISTS
...
<xsl:template match="iv:dbiv">
    ...
    <xsl:value-of select="concat('--  Generated by dbat/etc/xslt/dbiv_sproc.xsl V1.4 on ', translate(substring(date:date-time(), 1, 19), 'T', ' '), '&#10;')" />
    <xsl:value-of select="concat('DROP   PROCEDURE ', '')" />
    <xsl:choose>
        <xsl:when test="$rdbms = 'mysql'">
            <xsl:value-of select="'IF EXISTS '" />
        </xsl:when>
        <xsl:otherwise>
        </xsl:otherwise>
    </xsl:choose>
    <xsl:value-of select="concat(/iv:dbiv/iv:view/@proc, ' ', $delim, '&#10;')" />
