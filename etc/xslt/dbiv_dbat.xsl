...
<!--
    ...
    @(#) $Id: dbiv_spec.xsl 954 2012-06-03 15:52:18Z gfis $
...
<xsl:template match="iv:dbiv">
    ...
    <xsl:comment><!-- File header comment -->
        ...
        <xsl:value-of select="concat('&#9;Generated by dbat/etc/xslt/dbiv_spec.xsl V1.2 at ', date:date-time(), '&#10;')" />
<!--
-->
    ...
    <dbat   xmlns     ="http://www.teherba.org/2007/dbat"
        	...
        	<xsl:attribute name="title"   	><xsl:value-of select="@script"		/></xsl:attribute>
		    <xsl:value-of select='"&#10;&#9;"' />
...
<xsl:template match="iv:view">	
		...
		<choose>
		    ...
		    <when name="view" match="(sear|ins2|upd2|del2|)"> 
				...
				<ht:br />
		        <xsl:if test="$alter != 0"><!-- new entry form -->
				    <xsl:value-of select='"&#10;&#9;&#9;"' />
			        <ht:form>
			        	<xsl:attribute name="method"><xsl:value-of select="$method" /></xsl:attribute>
					    <xsl:value-of select='"&#10;&#9;&#9;&#9;"' />
			    ...
			    <xsl:value-of select='"&#10;&#9;&#9;"' />
		        <select limit="2048">
		            <xsl:for-each select="iv:action[@name != 'ins' and ($alter != 0 or $field/@name = 'dat')]">
...
<xsl:template name="wrap_callColumn">
	...
	<xsl:choose>
		<xsl:when test="string-length($field/@ref) != 0">
			<xsl:variable name="catalogFile" select="concat('../../web/spec/', substring-before(/iv:dbiv/@script, '/'), '/catalog.xml')" />
						...
						<xsl:choose>
							...
							<xsl:otherwise><!-- not enum, HTML input -->
						        <ht:input>
								    ...
								    <xsl:if test="count($field/iv:subquery) != 0">
			    				    	<xsl:attribute name="onblur">
											<!-- ajaxRequest(&apos;test.ajax02&amp;name=&apos; + this.form.name.value); -->
			    				    		<xsl:value-of select='concat("ajaxRequest(&apos;", $field/iv:subquery/@link)'  />
			    				    		<xsl:value-of select='concat("&apos; + this.form.", $field/@name, ".value);")' />
			    				    	</xsl:attribute>
				    				</xsl:if>
						    	</ht:input>
								<xsl:value-of select='concat(" ", $field/@remark)' />
